...
type Question = { id: string; topic: string; prompt: string; code?: string; options: string[]; answer: number; explanation: string; source: string; sourceUrl: string };
const questions: Question[] = [
  { id: 'q1', topic: 'Java · Exceptions', prompt: 'Which statement about a checked exception is true?', options: ['It always extends Error.', 'It must be caught or declared with throws.', 'It can never be thrown manually.', 'It is always caused by a syntax error.'], answer: 1, explanation: 'Checked exceptions are verified by the compiler. The method must handle them or include them in its throws clause.', source: 'Oracle Java docs', sourceUrl: 'https://docs.oracle.com/javase/tutorial/essential/exceptions/' },
  { id: 'q2', topic: 'Java · Type casting', prompt: 'What happens when this code runs?', code: 'Object value = "exam";\nInteger number = (Integer) value;', options: ['number becomes null.', 'It compiles and prints 0.', 'A ClassCastException is thrown.', 'The cast is silently ignored.'], answer: 2, explanation: 'The runtime object is a String, so an Integer cast fails at runtime. This matches Java downcasting behavior described in the language docs.', source: 'Oracle Java docs', sourceUrl: 'https://docs.oracle.com/javase/specs/' },
  { id: 'q3', topic: 'Java · Interfaces', prompt: 'Why can a class implement multiple interfaces?', options: ['Interfaces can provide multiple constructors.', 'Java allows multiple inheritance of type/contracts.', 'Interfaces always have instance fields.', 'The JVM merges their parent classes.'], answer: 1, explanation: 'A class can implement several interfaces to inherit multiple type contracts without inheriting state from multiple classes.', source: 'Oracle Java docs', sourceUrl: 'https://docs.oracle.com/javase/tutorial/java/IandI/createinterface.html' },
  { id: 'q4', topic: 'JavaScript · Closures', prompt: 'What is the main behavior of a closure in JavaScript?', options: ['It copies the entire runtime stack into a new function.', 'It keeps access to variables from the outer scope even after that scope has ended.', 'It prevents any variable reassignment in the parent function.', 'It can only be created inside classes.'], answer: 1, explanation: 'A closure preserves access to lexical variables from an outer function, even after the outer function returns. MDN explains this as a core behavioral feature of JavaScript functions.', source: 'MDN Web Docs', sourceUrl: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures' },
  { id: 'q5', topic: 'Java · Generics', prompt: 'Which collection can safely receive Integer values?', options: ['List<? extends Number>', 'List<? super Integer>', 'List<?>', 'List<Number> only'], answer: 1, explanation: 'A List<? super Integer> can accept Integer values because the wildcard allows Integer or supertypes, which is the safe consumer pattern described by Java generics rules.', source: 'Oracle Java docs', sourceUrl: 'https://docs.oracle.com/javase/tutorial/java/generics/wildcards.html' },
  { id: 'q6', topic: 'CSS · Flexbox', prompt: 'What does justify-content: center do in a flex container?', options: ['Moves items vertically to the center of the page.', 'Aligns items along the main axis within the container.', 'Makes the container item width shrink to fit.', 'Changes the flex direction to column.'], answer: 1, explanation: 'justify-content controls alignment along the main axis. MDN documents this as the central flexbox alignment property.', source: 'MDN Web Docs', sourceUrl: 'https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content' },
  { id: 'q7', topic: 'HTTP · Status codes', prompt: 'Which status code is most commonly used to indicate that a resource was found and the request succeeded?', options: ['201 Created', '302 Found', '200 OK', '404 Not Found'], answer: 2, explanation: '200 OK is the standard successful response for a request that completed successfully, as defined in HTTP semantics and documented by MDN.', source: 'MDN Web Docs', sourceUrl: 'https://developer.mozilla.org/en-US/docs/Web/HTTP/Status' },
  { id: 'q8', topic: 'Web APIs · fetch', prompt: 'What does fetch() return by default?', options: ['A parsed JSON object', 'A Promise that resolves to the Response object', 'A raw HTML string', 'A DOM element'], answer: 1, explanation: 'fetch() returns a Promise that resolves to a Response object. JSON parsing is a separate step with response.json().', source: 'MDN Web Docs', sourceUrl: 'https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch' },
];
...
function getExamTarget() {
  return new Date(2026, 10, 2, 0, 0, 0);
}
...
function Shell({ children }: { children: ReactNode }) {
  ...
  const [theme, setTheme] = usePersisted<'aurora' | 'grid' | 'starlight' | 'neon' | 'forest' | 'ember'>('java-background-theme', 'aurora');
  ...
  return (
    <div className={`noise theme-${theme} min-h-[100dvh] bg-background`}>
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 text-sidebar-foreground transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto rounded-2xl border border-sidebar-border bg-sidebar-accent/70 p-4">
          <div className="flex items-center justify-between gap-2 text-sidebar-foreground/70"><div className="flex items-center gap-2"><Target size={15} /><span className="mono text-[10px] uppercase tracking-[0.13em]">Exam window</span></div><span className="mono text-[9px] uppercase tracking-[0.12em] text-sidebar-foreground/60">Nov 2</span></div>
          <p className="mt-3 text-[12px] leading-5 text-sidebar-foreground/55">Final sprint target.<br />Keep the next block honest.</p>
  ...
  return <section className="group relative overflow-hidden rounded-[24px] bg-primary p-6 text-primary-foreground shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-8">
    ...
    <div className="relative flex min-h-[276px] flex-col items-center text-center">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        <span className="rounded-full bg-accent px-3 py-1.5 mono text-[10px] font-medium uppercase tracking-[0.12em] text-accent-foreground">Nov 2 · 00:00</span>
...
function TimeAllocationPlanner() {
  ...
  const updateHours = (id: string, hours: number) => {
    const sanitizedValue = Number.isFinite(hours) ? Math.max(0, hours) : 0;
    setAllocation((current) => {
      const updated = { ...current, [id]: sanitizedValue };
      const nextTotal = suggestedTimeAllocation.reduce((total, item) => total + (updated[item.id] ?? 0), 0);
      if (nextTotal <= budget) return updated;

      const otherTotal = suggestedTimeAllocation.reduce((total, item) => item.id === id ? total : total + (updated[item.id] ?? 0), 0);
      const maxAllowed = Math.max(0, budget - otherTotal);
      return { ...updated, [id]: Math.min(sanitizedValue, maxAllowed) };
    });
  };
  const reset = () => { setBudget(42); setAllocation(Object.fromEntries(suggestedTimeAllocation.map((item) => [item.id, item.hours]))); };
  return <section className="mt-5 rounded-[24px] border border-border bg-card p-5 shadow-sm sm:p-7"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2 text-muted-foreground"><Gauge size={16} /><span className="mono text-[10px] uppercase tracking-[0.16em]">Time allocation planner</span></div><h2 className="mt-2 display text-[23px] font-bold">Shape the week before it shapes you.</h2><p className="mt-2 max-w-2xl text-[13px] leading-6 text-muted-foreground">Adjust the sliders to split your weekly study hours across courses, projects, and recovery. Your plan stays saved in this browser.</p></div><button type="button" onClick={reset} data-testid="button-reset-time-allocation" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-border bg-background px-3 py-2.5 text-[12px] font-bold transition-colors hover:border-accent hover:bg-accent/10">Reset to suggested split</button></div><div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><div><div className="flex items-center justify-between gap-3"><label htmlFor="weekly-hour-budget" className="text-[13px] font-bold">Weekly hour budget</label><span className="mono text-[13px] font-bold text-accent-foreground">{budget}h</span></div><input id="weekly-hour-budget" type="range" min="0" max="80" step="1" value={budget} onChange={(event) => setBudget(Number(event.target.value))} data-testid="input-weekly-hour-budget" className="planner-range mt-3" style={{ '--range-progress': `${(budget / 80) * 100}%` } as React.CSSProperties} /><div className="mt-1 flex justify-between mono text-[9px] text-muted-foreground"><span>0h</span><span>80h</span></div><div className="mt-6 space-y-1.5">{suggestedTimeAllocation.map((item) => { const hours = allocation[item.id] ?? 0; return <div key={item.id} className="planner-control-row rounded-xl px-3 py-1.5"><div className="flex items-center justify-between gap-3"><label htmlFor={`allocation-${item.id}`} className="min-w-0 truncate text-[12px] font-semibold">{item.label}</label><span className="shrink-0 rounded-md bg-secondary px-2 py-0.5 mono text-[10px] font-bold text-secondary-foreground">{hours}h</span></div><input id={`allocation-${item.id}`} type="range" min="0" max="20" step="1" value={hours} onChange={(event) => updateHours(item.id, Number(event.target.value))} data-testid={`input-allocation-${item.id}`} aria-label={`${item.label} hours per week`} className="planner-range mt-1" style={{ '--range-progress': `${(hours / 20) * 100}%` } as React.CSSProperties} /></div>; })}</div></div><div className="rounded-2xl border border-border bg-background/45 p-4"><div className="flex items-center justify-between gap-3"><div><div className="mono text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Live split</div><div className="mt-1 display text-[25px] font-bold">{totalHours}h <span className="text-[13px] font-medium text-muted-foreground">planned</span></div></div><span className={`rounded-full px-3 py-1 mono text-[10px] font-bold ${difference === 0 ? 'bg-[#4f9c7a]/15 text-[#3f7559]' : difference > 0 ? 'bg-[#e66b5d]/15 text-[#b94a40]' : 'bg-accent/20 text-accent-foreground'}`}>{status}</span></div><div className="mt-5 flex h-10 w-full overflow-hidden rounded-xl bg-secondary p-1" aria-label="Proportional weekly time allocation chart" role="img">{suggestedTimeAllocation.map((item) => { const hours = allocation[item.id] ?? 0; return hours > 0 ? <div key={item.id} title={`${item.label}: ${hours}h`} className="planner-chart-segment first:rounded-l-lg last:rounded-r-lg" style={{ width: `${totalHours ? (hours / totalHours) * 100 : 0}%`, backgroundColor: item.color }} /> : null; })}</div><div className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2">{suggestedTimeAllocation.map((item) => <div key={item.id} className="flex min-w-0 items-center gap-2 text-[11px]"><span className="size-2.5 shrink-0 rounded-sm" style={{ backgroundColor: item.color }} /><span className="truncate text-muted-foreground">{item.label}</span><span className="ml-auto mono text-[10px]">{allocation[item.id] ?? 0}h</span></div>)}</div><div className="mt-5 grid grid-cols-2 gap-2 border-t border-border pt-4"><div className="rounded-xl bg-secondary/60 px-3 py-2"><div className="mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">Budget used</div><div className="mt-1 text-[14px] font-bold">{budget ? Math.round((totalHours / budget) * 100) : 0}%</div></div><div className="rounded-xl bg-secondary/60 px-3 py-2"><div className="mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">Remaining</div><div className="mt-1 text-[14px] font-bold">{budget - totalHours}h</div></div><p className="col-span-2 text-[12px] leading-5 text-muted-foreground">{difference > 0 ? 'Trim the plan or raise the budget before the week begins.' : difference < 0 ? 'You have room for more study, project depth, or recovery.' : 'The split matches your weekly budget.'}</p></div></div></div></section>;
}
...
function Practice() {
  ...
  return <div className="rise">
    <SectionIntro kicker="Practice · real-world fundamentals" title="Study from the source." detail="These questions are grounded in practical web documentation and official language references so the drill stays aligned with what engineers actually use." action={<div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2"><Trophy size={16} className="text-[#d19a39]" /><span data-testid="text-practice-score" className="mono text-[12px] font-medium">{score}/{Object.keys(results).length || 0} correct</span></div>} />
